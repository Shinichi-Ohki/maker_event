...
- 🗺️ **Data-driven Country Detection** - Automatic country detection from spreadsheet data
...
- 🗺️ **データドリブン国判定** - スプレッドシートデータから自動的に国を判定
...
### Country Detection | 国判定

The script automatically detects countries from the "地域" (Region) column in the spreadsheet. Use the following format:

```
都市名(国名)  # e.g., パリ(フランス), サンフランシスコ(アメリカ)
都市名(都道府県) # e.g., 武蔵小杉(神奈川), 長岡(新潟)
```

Country and prefecture names are mapped using `country_mapping.json`:
- All 47 Japanese prefectures are automatically recognized as Japan
- 40+ international countries are supported
- To add new countries, simply add an entry to `country_mapping.json`

スクリプトはスプレッドシートの「地域」列から自動的に国を検出します。以下の形式を使用してください：

```
都市名(国名)  # 例: パリ(フランス), サンフランシスコ(アメリカ)
都市名(都道府県) # 例: 武蔵小杉(神奈川), 長岡(新潟)
```

国名・都道府県名は`country_mapping.json`を使用してマッピングされます：
- 日本の47都道府県は全て自動的に日本として認識されます
- 40カ国以上の海外の国に対応しています
- 新しい国を追加するには、`country_mapping.json`にエントリを追加するだけです
...
├── country_mapping.json       # Country/prefecture name mapping | 国名・都道府県名マッピング
